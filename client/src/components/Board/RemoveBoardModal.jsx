...
const RemoveBoardModal = ({ isOpen, onClose, boardId }) => {
    ...
    const handleRemove = async () => {
        ...
        try {
            ...
            setStatus({
                success: false,
                message: 'Failed to remove board, try again later'
            });
        } finally {
            setLoading(false);
        }
        ...
        <Modal isOpen={isOpen} onClose={handleClose} title="Remove Board">
            <div className="mb-4 p-4 bg-red-50 border border-red-200 rounded-lg">
                <p className="text-red-800 text-sm">
                    <strong>Warning:</strong> Removing this board will
                    permanently delete all its data. This action cannot be
                    undone.
                </p>
            </div>
            ...
            {status.message && (
                <div
                    className={`mt-4 mb-6 text-sm ${status.success ? 'text-green-500' : 'text-red-500'}`}
                >
                    {status.message}
                </div>
            )}
            ...
            <div className="flex gap-3 justify-end">
                ...
                <button
                    ...
                    className={`py-2 px-4 rounded-lg focus:outline-none focus:ring-0 transition-colors ${
                        confirmationText === 'DEACTIVATE' && !loading
