            ...
            >
                ...
                {isDraggable && (
                    <div className="absolute left-1 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-30 transition-opacity">
                        <svg
                            className="w-3 h-5 text-gray-400"
                            fill="currentColor"
                            viewBox="0 0 8 20"
                        >
                ...
                <div className="pr-6">
                    ...
                    <div className="mb-2 flex items-start gap-3">
                        ...
                        />
                        <h4
                            className={`font-medium text-sm text-gray-900 line-clamp-2 flex-1 ${isCompleted ? 'line-through text-gray-500' : ''}`}
                        >
                    ...
                    {task.description && (
                        <div className="ml-7">
                            <p
                                className={`text-xs text-gray-600 line-clamp-2 mb-3 ${isCompleted ? 'line-through text-gray-400' : ''}`}
                            >
                                {task.description}
                            </p>
                        </div>
                    ...
                    <div className="space-y-2 ml-7">
                        ...
                        <div className="flex items-center justify-between">
                            {task.priority && (
                                <span
                                    className={`text-xs px-2 py-1 rounded-full font-medium ${getPriorityColor(task.priority)} ${isCompleted ? 'opacity-60' : ''}`}
                                >
                            ...
                            {dueDateInfo && (
                                <span
                                    className={`text-xs ${dueDateInfo.className} ${isCompleted ? 'opacity-60' : ''}`}
                                >
                                    {dueDateInfo.text}
                                </span>
                            )}
                        ...
                        {task.assignees && task.assignees.length > 0 && (
                            <div className="flex items-center justify-between">
                                <div className="flex items-center">
                                    <svg
                                        className={`w-3 h-3 text-gray-400 mr-1 ${isCompleted ? 'opacity-60' : ''}`}
                                        fill="none"
                                        stroke="currentColor"
                                        viewBox="0 0 24 24"
                                    >
                                    ...
                                    <span
                                        className={`text-xs text-gray-500 mr-2 ${isCompleted ? 'opacity-60' : ''}`}
                                    >
                                        Assigned to:
                                    </span>
                                </div>
                                <div
                                    className={`flex -space-x-1 ${isCompleted ? 'opacity-60' : ''}`}
                                >
                                    {task.assignees
                                        .slice(0, 3)
                                        .map((assignee, index) => (
