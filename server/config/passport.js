...
module.exports = function (passport) {
  ...
  passport.use(
    new GitHubStrategy(
      ...
      async (accessToken, refreshToken, profile, done) => {
        try {
          console.log('GitHub Profile:', JSON.stringify(profile, null, 2)); // Debug log
          
          const email = profile.emails?.[0]?.value;
          
          // Handle missing email
          if (!email) {
            return done(new Error('No email provided by GitHub. Please make sure your GitHub account has a public email address.'), null);
          }
