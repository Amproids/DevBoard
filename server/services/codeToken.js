...
const createCodeTokenService = async email => {
    try {
        ...
        setTimeout(
            async () => {
                try {
                    ...
                    console.error(
                        `Error auto-removing code token for ${email}:`,
                        err.message
                    );
                }
            },
        ...
        console.error(
            'Error in codeToken.service.js -> createCodeTokenService:',
            err.message
        );
...
const verifyCodeTokenService = async (email, token) => {
    try {
        ...
        console.error(
            'Error in codeToken.service.js -> verifyCodeTokenService:',
            err.message
        );
        if (err.status === 401) {
            throw err;
...
const updatePassword = async (email, newPassword) => {
    try {
        ...
        console.error(
            'Error in codeToken.service.js -> updatePassword:',
            err.message
        );
